...
function renderDashboard() {
  ...
  if (sbInput && !sbInput._bound) {
    sbInput._bound = true;
    // Load saved value
    const saved = getStartingBalance();
    if (saved) sbInput.value = saved;
    ...
    // Eye toggle
    const eyeBtn = document.getElementById('sb-eye-btn');
    if (eyeBtn) {
      eyeBtn.addEventListener('click', () => {
        const isHidden = sbInput.type === 'password';
        sbInput.type = isHidden ? 'text' : 'password';
